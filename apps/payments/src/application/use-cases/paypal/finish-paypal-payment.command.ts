  ...
  async execute(payload: FinishPaypalPaymentCommand) {
    try {
      ...
      if(payload.body.payload.event_type === 'PAYMENT.SALE.COMPLETED') {
        ...
        if(await this.subscriptionRepo.getSubscriptionByPaypalSubId(paypalSubscriptionId)) return

        const { plan, userId } =
          await this.paypalAdapter.getPaypalSubscriptionInfo(this.token, paypalSubscriptionId)

        const subscriptionDto = CreateSubscriptionDto.createSubscriptionDto(
          plan, 'Paypal', paypalSubscriptionId, +userId
        )

        const subscription = SubscriptionEntity.create(subscriptionDto)
        const payment = PaymentsEntity.create('Paypal', plan, subscription.dateOfNextPayment, +userId)
        ...
        const newExpirationDate = SubscriptionEntity.getNewExpireAt(currentSubscription.expireAt, subscription.period)
        ...
        return { userId: +userId, email: currentSubscription.profile.user.email}
      }
