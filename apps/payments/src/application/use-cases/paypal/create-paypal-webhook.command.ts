import { AddNewSubscriptionTypeDto, CreatePaypalWebhookDto } from "../../../api/dto/product.dto";
...
export class CreatePaypalWebhookHandler
  ...
  async execute(payload: CreatePaypalWebhookCommand) {
    try {
      ...
      const webhookResponse = await fetch('https://api-m.sandbox.paypal.com/v1/notifications/webhooks', {
        method: 'POST',
        headers: {
